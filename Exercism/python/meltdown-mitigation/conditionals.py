...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    if (generated_power/theoretical_max_power)*100 >= 80:
        return 'green'
    if 60 <= (generated_power/theoretical_max_power)*100 < 80:
        return 'orange'
    if 30 <= (generated_power/theoretical_max_power)*100 < 60:
        return 'red'
    return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if state < (90/100*threshold):
        return 'LOW'
    if (threshold*90/100) < state < (110/100*threshold):
        return 'NORMAL'
    return 'DANGER'